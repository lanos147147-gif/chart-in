        ...
        tickerBtn.addEventListener("click", () => {
            ...
            if (marketMode === "kr" && marketSelect) {
                if (ticker.endsWith(".KQ")) {
                    marketSelect.value = "KQ";
                } else {
                    marketSelect.value = "KS";
                }
            }
            analyzeTicker();
        });
...
function renderSummary(data) {
    ...
    tickerName.textContent = `${data.display_ticker || data.ticker} · ${data.market}`;
    ...
    priceValue.textContent = formatPrice(data.summary.close, currentCurrency);
